...
- **New user or staff member:** read the [complete User Manual](docs/USER-MANUAL.md).
- **Developer or administrator:** follow the [free-tier Setup Guide](docs/SETUP-GUIDE.md).
- **Security reviewer:** read the [Security Architecture](SECURITY-ARCHITECTURE.md).
...
Open the URL printed in the terminal. Localhost uses demonstration data by default; add `?live=1` to test Firebase. Before a real deployment, complete every step in the Setup Guide rather than deploying with example settings.
...
Localhost starts in demonstration mode by default. Production deployments never substitute local demo medical data when Firebase is unavailable. Use `?live=1` on localhost to test the configured Firebase project, or `?demo=1` to request the demo explicitly. All demo accounts use `demo1234`:
...
Production biometric setup requires a trusted backend, certified hardware/provider, governance, and billing; it is disabled in the free Spark build:

1. Enable Firebase App Check with reCAPTCHA v3 for the web app and set the public `appCheckRecaptchaSiteKey` in `js/firebase-config.js`.
2. Have an administrator create verified staff documents with `role`, `staffStatus: "active"`, and `organisationId`. Staff roles cannot be self-registered.
3. Configure a certified scanner bridge that exposes `window.GEDIC_SCANNER.capture(...)` and returns only a signed capture token.
4. Configure the HTTPS matching gateway secrets:

   ```powershell
   firebase functions:secrets:set BIOMETRIC_MATCH_URL
   firebase functions:secrets:set BIOMETRIC_API_KEY
   firebase functions:secrets:set BIOMETRIC_GATEWAY_SIGNING_KEY
   ```

5. Deploy with `firebase.blaze.json`, then set `GEDIC_FEATURES.biometricEmergencyAccess` to `true` in `js/firebase-config.js`.

The backend issues 2-minute one-time nonces, allows five challenges per responder per ten minutes, binds each scan to the responder/reason/incident, returns only allow-listed emergency fields, and writes server-only audit records. The gateway must verify scanner signatures, liveness, nonce freshness, and device registration. Use mutual TLS or a private managed connection in production.
...
6. Deploy the included security rules and care-team query index:
...
| --- | --- | --- |
| `users/{uid}` | Account and immutable authorisation attributes | No |
| `patients/{documentId}` | Authenticated clinical dashboard record | No |
| `publicProfiles/{randomToken}` | Patient-consented emergency summary opened by a revocable QR | Direct lookup only; collection listing is denied |

The QR contains only `index.html?view=<192-bit-random-token>`. It does not expose a patient UID or embed medical information. Sharing is opt-in, and the patient can disable it or rotate the token to revoke old links. Older accounts default to sharing disabled and should review the consent control after signing in.
...
### Free staff administration

Staff cannot self-select a trusted role. Using a local administrator machine and a Google service-account credential is free; the Admin SDK bypasses client rules, so protect that credential and never place it in this repository or a browser.

Create the professional's account in Firebase Authentication, verify its email, then run:
...
Hospital users can read organisation-linked patients. Doctors can read only patients whose `careTeamUids` includes their UID. Neither role can change assignment, ownership, QR consent, or the QR token through the client.
...
- Published demo emails use the local demonstration database only on localhost or when `?demo=1` is present.
...
2. Complete the medical profile, enable the explicit public emergency-sharing checkbox, and save it.
...
The public emergency document can contain health and contact information because unauthenticated QR scanning is the purpose of GEDIC. Obtain explicit patient consent, minimize collected data, publish a privacy policy, and review the healthcare and privacy laws that apply to the deployment location. The record is labelled patient-reported or clinician-verified and shows its update time, but GEDIC is not a certified medical device, a substitute for clinical records, medical advice, or local emergency services.
