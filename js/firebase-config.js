...
const GEDIC_FEATURES = Object.freeze({
  ...
  // Requires Firebase App Check, the Blaze functions deployment, and a
  // certified scanner gateway. The browser never receives biometric samples.
  biometricEmergencyAccess: false,
  // Public reCAPTCHA v3 site key used by Firebase App Check (not a secret).
...
function initFirebase() {
  ...
  try {
    ...
    if (GEDIC_FEATURES.biometricEmergencyAccess && !GEDIC_FEATURES.appCheckRecaptchaSiteKey) {
      throw new Error("App Check site key is required for biometric emergency access.");
    }
    if (GEDIC_FEATURES.appCheckRecaptchaSiteKey) {
      firebase.appCheck().activate(GEDIC_FEATURES.appCheckRecaptchaSiteKey, true);
    }
    _auth = firebase.auth();
