...
const Hospital = (() => {
  ...
  function _renderTable(pts) {
    ...
    tbody.innerHTML = pts.map(p => `
      <tr>
        ...
        <td><span class="blood-tag" style="font-size:.72rem;padding:1px 8px">${e(p.blood||"—")}</span></td>
        <td>${e(p.age||"—")}</td>
        <td class="col-amber" style="max-width:140px">${e(UI.trunc(p.diseases,32))}</td>
  ...
  async function save() {
    ...
    const data = {
      ...
      updatedAt: Date.now(), dataStatus: "clinician-verified"
    };
    ...
    try {
      if (_editId) {
        ...
        UI.toast("Patient record updated. The patient controls republication of their public QR summary.", "ok");
      } else throw new Error("Patients must create their own account and consent before a hospital can access their record.");
