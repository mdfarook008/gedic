...
const PrintCard = (() => {
  ...
  function generate(profile) {
    ...
    const link = getEmergencyURL(profile.emergencyToken);
...
<head>
<meta charset="UTF-8">
<title>GEDIC Emergency Card — ${e(profile.name || "Patient")}</title>
<style>
...
<div class="print-controls">
  <button class="btn-print" id="printAction">🖨️ Print Card</button>
  <button class="btn-close" id="closeAction">✕ Close</button>
</div>
...
<div class="card-wrap">
  ...
  <div class="card-front">
    <div>
      ...
      <div class="patient-name">${e(profile.name || "—")}</div>
      <div class="blood-badge">${e(profile.blood || "—")}</div>
      <div class="info-row"><strong>Age:</strong>${e(profile.age || "—")}</div>
      <div class="info-row"><strong>Hospital:</strong>${e(profile.hospital || "—")}</div>
      <div class="info-row"><strong>Doctor:</strong>${e(profile.doctorName || "—")}</div>
      <div class="info-row"><strong>Dr. Ph:</strong>${e(ph(profile.doctorPhone))}</div>
      <div class="info-row"><strong>Emergency:</strong>${e(profile.emergencyName || "—")}</div>
      <div class="info-row"><strong>Ph:</strong>${e(ph(profile.emergencyContact))}</div>
      ${profile.medicines ? `<div class="info-row"><strong>Meds:</strong>${e(profile.medicines)}</div>` : ""}
    </div>
    ${profile.allergies ? `<div class="allergy-warn">⚠️ ALLERGIES: ${e(profile.allergies)}</div>` : ""}
  </div>
  ...
  <div class="card-back">
    <div style="width:100%;text-align:center">
      ...
      <div class="qr-url">${e(link)}</div>
    </div>
    <div class="contacts">
      <div class="contact-row"><span>Condition:</span> ${e(profile.diseases || "—")}</div>
      <div class="contact-row"><span>Allergy:</span> ${e(profile.allergies || "None")}</div>
    </div>
    ...
    win.document.close();
    win.document.getElementById("printAction")?.addEventListener("click", () => win.print());
    win.document.getElementById("closeAction")?.addEventListener("click", () => win.close());
  }
