...
test('seeds a responder and records a local demo break-glass audit', () => {
  const db = createDatabase();
  db('DB.seed()');
  assert.equal(db(`DB.login('responder@gedic.app', 'demo1234').role`), 'responder');
  const id = db(`DB.recordEmergencyAccess({actorUid:'demo-uid-r1', patientUid:'demo-uid-p1', reason:'Road accident emergency', incidentRef:'EMS-42'}).id`);
  assert.match(id, /^demo-audit-/);
  assert.equal(db(`JSON.parse(localStorage.getItem('gedic_db_v3')).emergencyAccessLogs.length`), 1);
});
